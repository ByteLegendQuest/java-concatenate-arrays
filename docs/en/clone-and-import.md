...
`git clone https://github.com/ByteLegendQuest/java-concatenate-arrays`

A project directory named `java-concatenate-arrays` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-concatenate-arrays` directory which is created just now.
